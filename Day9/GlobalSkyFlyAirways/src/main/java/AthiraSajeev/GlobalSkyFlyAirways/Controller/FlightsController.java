...
public class FlightsController {
    ...
    public String airlineRefund(@PathVariable String airlineId, Model model) {
        ...
        ArrayList<Tickets> userCancelledTicketList = new ArrayList<Tickets>();
        for (Flights flight : flights) {
            if (flight.getAirlines().getUsername().contains(airlineId) && flight.getCancelStatus()) {
        ...
        for (Flights flight : flights) {
            if (flight.getAirlines().getUsername().contains(airlineId) && !flight.getCancelStatus()) {
                List<Tickets> tickets = ticketsService.getTicketsByFlightId(flight.getFlightId());
                for (Tickets ticket : tickets) {
                    if (!ticket.getRepaymentStatus() && ticket.getTicketStatus().contains("Cancelled")) {
                        userCancelledTicketList.add(ticket);
                    }
                }
            }
        }

        model.addAttribute("userCancelledTicketList", userCancelledTicketList);
    ...
    public String airlineRefund(Model model, HttpServletRequest request) {
        ...
        for (Flights flightInstance : flightsList) {
            if (flightInstance.getAirlines().getUsername().contains(airlineId) && !flightInstance.getCancelStatus()) {
                List<Tickets> tickets = ticketsService.getTicketsByFlightId(flightInstance.getFlightId());
                for (Tickets ticketInstance : tickets) {
                    if (!ticketInstance.getRepaymentStatus() && ticketInstance.getTicketStatus().contains("Cancelled")) {
                        userCancelledTicketList.add(ticketInstance);
                    }
                }
            }
        }
    ...
    @RequestMapping("/bookedFlightSeats")
    public String bookedFlightSeats(Model model) {
        model.addAttribute("airline", airlinesService.getAirlineById(airlineID));
        model.addAttribute("airlineId", airlineID);
        return "bookedFlightSeats";
    }

    @PostMapping("/bookedFlightSeats")
    public String bookedFlightSeats(Model model,HttpServletRequest request) {
        String flightId=request.getParameter("flightId");
        airlineID=flightsService.getFlightById(flightId).getAirlines().getUsername();
        List<Tickets> tickets=ticketsService.getTicketsByFlightId(flightId);
